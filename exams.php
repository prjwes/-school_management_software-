...
if (isset($_GET['export_csv']) && isset($_GET['exam_id'])) {
    ...
    if ($exam) {
        ...
        fputcsv($output, ['Admission Number', 'Student Name', 'Subject', 'Marks', 'Rubric']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['import_excel'])) {
    if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] === 0) {
        ...
        // Simple CSV parsing (for Excel saved as CSV)
        if (($handle = fopen($file, 'r')) !== FALSE) {
            $row_num = 0;
            while (($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
                $row_num++;
                if ($row_num < 4) continue; // Skip header rows, start from row 4
                
                if (empty($data[0])) break; // Stop at empty row
                ...
                if ($student) {
                    // Import marks for each subject (starting from column 2)
                    $subjects = ['English', 'Kiswahili', 'Mathematics', 'Integrated Science', 'CRE', 'CA&S', 'Pre-technical Studies', 'Social Studies', 'Agriculture'];
                    
                    foreach ($subjects as $idx => $subject) {
                        $col_idx = 2 + $idx;
                        if (isset($data[$col_idx]) && !empty($data[$col_idx])) {
                            $marks = floatval($data[$col_idx]);
                            ...
                            $stmt->bind_param("isiddd", $exam_id, $student['id'], $subject, $marks, $marks);
                            $stmt->execute();
                            $stmt->close();
                        }
                    }
